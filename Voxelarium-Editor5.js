...
function init() {
	...
	if (!Voxelarium.Settings.VR) {
		...
		if (!Voxelarium.Settings.use_basic_material) {
			glow.makeComposers(renderer, scene, function () {
				clusters.forEach(function (cluster) {
					cluster.SectorList.forEach(function (sector) {
		...
		if (!Voxelarium.Settings.AltSpace) {
			...
			if (!Voxelarium.Settings.use_basic_material) {
				glow.makeComposers(effect, scene, function () {
					clusters.forEach(function (cluster) {
						cluster.SectorList.forEach(function (sector) {
							sector.solid_geometry.geometry.uniforms.edge_only = 0;
						});
...
function render() {
	if (Voxelarium.Settings.ALtSpace) return;
	if (Voxelarium.Settings.VR) {
...
function animate() {
	...
	if (slow_animate) requestAnimationFrame(slowanim);else {
		if (Voxelarium.Settings.VR) vrDisplay.requestAnimationFrame(animate);else requestAnimationFrame(animate);
	}
...
function initVoxelarium() {
	...
	var geometryMaterial = Voxelarium.Settings.use_basic_material ? Voxelarium.GeometryBasicBuffer() : Voxelarium.GeometryBuffer();

	geometryShader = Voxelarium.Settings.use_basic_material ? new THREE.MeshBasicMaterial() : Voxelarium.GeometryShader();

	Voxelarium.TextureAtlas.init(32, 64);

	Voxelarium.db.init(function () {
		//geometryShader.uniforms.map.value = Voxelarium.TextureAtlas.texture;
		geometryShader.vertexColors = THREE.VertexColors;
		if (geometryShader.uniforms) geometryShader.uniforms.map.value = Voxelarium.TextureAtlas.texture;else geometryShader.map = Voxelarium.TextureAtlas.texture;
		...
		cluster.pivot.add(THREE.Vector3Pool.new(cluster.voxelUnitSize * (cluster.sectorSizeX / 2), -1 * cluster.voxelUnitSize * (cluster.sectorSizeY / 2), cluster.voxelUnitSize * (cluster.sectorSizeZ / 2)).delete());
		if (controlOrbit) controlOrbit.center = cluster.pivot;
		...
		var inventory_geometryShader = Voxelarium.Settings.use_basic_material ? new THREE.MeshBasicMaterial() : Voxelarium.GeometryShader();
		...
		if (inventory_geometryShader.uniforms) inventory_geometryShader.uniforms.map.value = Voxelarium.TextureAtlas.texture;
		...
		//stats.begin();
		if (!Voxelarium.Settings.AltSpace) animate();
	});
	//});
...
//# sourceMappingURL=Voxelarium-Editor5.js.map