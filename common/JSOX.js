...
JSOX.begin = function( cb, reviver ) {
	...
	return {
		...
		registerFromJSOX( prototypeName, o, f ) {
                	throw new Error( "registerFromJSOX is deprecated, please update to use fromJSOX instead" );
                },
		finalError() {
		...
		_write(msg,complete_at_end) {
			...
			function convertValue() {
				...
				case VALUE_ARRAY:
					...
					if( arrayType >= 0 ) {
						...
						var obj = rootObject;
						if( _DEBUG_PARSING_DETAILS ) {
							console.log( "Resolve path:", val,"in", obj );
							val.contains.reduce( (acc,part)=>{
								console.log( "Get value in :", part, Object.getPrototypeOf(acc), Map.prototype, acc[part] );
								
								if( Object.getPrototypeOf(acc) === Map.prototype ) {
									//console.log( "Is a map... use getter", acc.get(part) );
									return acc.get( part );
								}
								//console.log( "Using normal method...", acc );
								return acc && acc[part];
							} , obj );
						}
						obj = val.contains.reduce( (acc,part)=>( Object.getPrototypeOf(acc) === Map.prototype )?acc.get(part):acc && (acc = acc[part]), obj );
			...
			function arrayPush() {
				...
				if( arrayType == -3 )  {
					_DEBUG_PARSING && console.log(" Array type -3?" );
					if( val.value_type === VALUE_OBJECT ) {
						element_array[element_array.length-1] = val.contains;
					}
			...
			function objectPush() {
				if( arrayType === -3 ) {
					...
					_DEBUG_PARSING_DETAILS && console.log( "ABORTING UPDATE TO OBJECT: ALREADY UPDATED:" );
					arrayType = -1;
					return;
				}
			...
			function recoverIdent(cInt) {
				_DEBUG_PARSING&&console.trace( "Recover Ident char:", cInt, String.fromCodePoint(cInt), "word:", word );
				if( word !== WORD_POS_RESET ) {
					if( negative ) { 
						valstring += "-"; negative = false; 
			...
			function openObject() {
				...
				if( parse_context == CONTEXT_UNKNOWN ) {
					if( word == WORD_POS_FIELD /*|| word == WORD_POS_AFTER_FIELD*/ && val.string.length ) {
...

						{
							...
							if( !cls ) {			
								...
								classes.push( cls = { name : val.string, protoCon: (protoDef && protoDef.protoCon) || privateProto.constructor, fields : [] } );
					...
					if( word != WORD_POS_RESET || val.value_type == VALUE_STRING ) {
						var protoDef;
						if( protoDef = localFromProtoTypes.get( val.string ) ) {
							val.className = val.string;
							// need to collect the object, 
							tmpobj = {};//new protoDef.protoCon();
						}
						else if( protoDef = fromProtoTypes.get( val.string ) ) {
							val.className = val.string;
							tmpobj = {};//new protoDef.protoCon();
						} else {
							cls = classes.find( cls=>cls.name === val.string );
			...
			function openArray() {
				...
				if( word == WORD_POS_END && val.string.length ) {
					if( _DEBUG_PARSING ) console.log( "recover arrayType:", arrayType, val.string );
				...
				}
				{
					...
					if( parse_context == CONTEXT_UNKNOWN )
						...
						_DEBUG_PARSING_DETAILS && console.log( "adding array to existing object as name: ", val.name );
					...
					arrayType = -1;
					val.className = null;
					_DEBUG_PARSING_DETAILS && console.log( " !!!!!B Clear Classname:", val.className, old_context.className, old_context.name );
					val.name = null;
					current_proto = null;
					current_class = null;
					current_class_field = 0;
			...
			function getProto() {
				...
				} 
				{
					...
					if( !result.protoDef && !result.cls ) throwError( "Referenced class " + val.string + " has not been defined", cInt );
				}
			...
			while( status && ( input = inQueue.shift() ) ) {
				...
				while( !completed && status && ( n < buf.length ) ) {
					...
					if( _DEBUG_LL ) console.log( "processing: ", cInt, n, str, pos, comment, parse_context, word );
					pos.col++;
					if( comment ) {
						if( comment == 1 ) {
					...
					case 58/*':'*/:
						...
						break;
					case 125/*'}'*/:
						if(_DEBUG_PARSING) console.log( "close bracket context:", word, parse_context, val.value_type, val.string );
					...
					case 44/*','*/:
						...
						if( parse_context == CONTEXT_CLASS_FIELD ) {
							...
							if( _DEBUG_PARSING ) console.log( "comma after field value, push field to object: %s", val.name, val.value_type );
					...
					default:
						...
						case 0x2029: // PS (paragraph separate)
						case 0xFEFF://'\uFEFF':
							_DEBUG_PARSING && console.log( "Whitespace...", word, parse_context );
							if( word == WORD_POS_END ) {
								if( parse_context == CONTEXT_UNKNOWN ) {
									word = WORD_POS_RESET;
									completed = true;
...
JSOX.parse = function( msg, reviver ) {
	...
	if( writeResult > 0 ) {
		...
		var reuslt = typeof reviver === 'function' ? (function walk(holder, key) {
			...
			console.log( "SO this calls deeply?", key, value)
			return reviver.call(holder, key, value);
		}({'': result}, '')) : result;
		_parse_level--;
		return result;
...
/* init prototypes */
{
	...
	fromProtoTypes.set( "map", { protoCon:Map.constructor, cb:function (field,val){
		if( field ) this.set( field, val );
		return this;
	} } );
...
JSOX.registerToJSOX = function( name, prototype, f ) {
	...
	if( !prototype.prototype || prototype.prototype !== Object.prototype ) {
		...
		toProtoTypes.set( prototype.prototype, { external:true, name:name||f.constructor.name, cb:f } );
	} else {
...
JSOX.registerFromJSOX = function( prototypeName, o, f ) {
	throw new Error( "deprecated; please adjust code to use fromJSOX" );
	if( fromProtoTypes.get(prototypeName) ) throw new Error( "Existing fromJSOX has been registered for prototype" );
...
JSOX.registerToFrom = function( prototypeName, prototype, to, from ) {
	throw new Error( "registerToFrom deprecated; please use addType" );
		...
		// Produce a string from holder[key].
			...
			if( value !== undefined
				...
				if( typeof value === "object" ) {
					...
					_DEBUG_PARSING_DETAILS && console.log( "This object is not yet an tracked object path:", v, value  );
					if( v ) return "ref"+v;
				}
