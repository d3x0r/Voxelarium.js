...
class World {
		...
		createTextCursor( opts ){
			...
			const voxelUnitSize = opts.voxelUnitSize || 1;
			const v = opts.voxelType || Voxelarium.Voxels.BlackRockType
            var cluster = Voxelarium.Cluster(8,8,1);
            cluster.mesher = opts.mesher ;
        ...
        createDynamicTextCluster( text, opts ) {
			...
			let voxelTypeCache_ = typeCache.get(v);
			if( !voxelTypeCache_ ) typeCache.set(v,voxelTypeCache_ = []);
			let voxelTypeCache = voxelTypeCache_.find( vtc=>vtc.voxelUnitSize===voxelUnitSize)
			if( !voxelTypeCache ) voxelTypeCache_.push( voxelTypeCache = {voxelUnitSize:voxelUnitSize,cache:{}})
			voxelTypeCache = voxelTypeCache.cache
