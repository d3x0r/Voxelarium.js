...
function render() {
	...
	if( u  ){
		...
		const ang2 = Number(chkLockD.checked?sldDir1.value:sldDir2.value)/360 * 2*Math.PI
...
function initVoxelarium() {
	...
	Voxelarium.geometryShader = geometryShader = Voxelarium.Settings.use_basic_material
...
function initAltSpace( init ) {
	...
	Voxelarium.renderer = renderer = sim.renderer;
